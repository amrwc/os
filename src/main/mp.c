...
static struct mp* mpsearch1(uint32_t a, int len)
{
	uint8_t *e, *p, *addr;
...
void mpinit(void)
{
	...
	ismp = 1;
	localApic = (uint32_t*)conf->lapicaddr;
	...
	if (mp->imcrp) 
	{
		...
		outputByteToPort(0x22, 0x70);   // Select IMCR
		outputByteToPort(0x23, inputByteFromPort(0x23) | 1);  // Mask external interrupts.
	}
}
